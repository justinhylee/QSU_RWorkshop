...
# saveRDS(tab1, "/Users/jlee/Library/CloudStorage/Box-Box/jlee/QSU_RWorkshop/data/tab1.rds")
```
...
df_game <- gamelog_mookie %>%
  dplyr::select(Rk, Rslt, PA, AB, R, H, HR, RBI, BB, OBP) %>%
  rename("Game"="Rk") %>%
...
df_mod <- gamelog_mookie %>%
  ...
  dplyr::select(Date, month, Win, PA, PA.num, H, H.num, SO, SO.num)
